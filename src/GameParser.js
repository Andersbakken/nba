...
function parseQuarters(league, net, data) {
    return new Promise(function(resolve) {
        ...
        var homePlayerData, awayPlayerData;
        var playerDataIndexes = {};
        var errors = [];

        var rosters = Promise.all([ net.get(`http://stats.nba.com/stats/commonteamroster/?TeamId=${home.id}&Season=${season}`),
                                    net.get(`http://stats.nba.com/stats/commonteamroster/?TeamId=${away.id}&Season=${season}`) ]);
        rosters.then(function(results) {
            homePlayerData = JSON.parse(results[0].body);
            for (var i=0; i<homePlayerData.resultSets[0].headers.length; ++i) {
                playerDataIndexes[homePlayerData.resultSets[0].headers[i]] = i;
            }
            awayPlayerData = JSON.parse(results[1].body);
        }).then(function() {
